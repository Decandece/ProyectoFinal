...
CREATE TABLE users (
    ...
    phone VARCHAR(20),
    role VARCHAR(20) NOT NULL CHECK (role IN ('PASSENGER', 'CLERK', 'DRIVER', 'DISPATCHER', 'ADMIN')),
...
CREATE TABLE trips (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'SCHEDULED' CHECK (status IN ('SCHEDULED', 'BOARDING', 'DEPARTED', 'ARRIVED', 'CANCELLED')),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE incidents (
    ...
    incident_type VARCHAR(30) NOT NULL CHECK (incident_type IN ('SECURITY', 'DELIVERY_FAIL', 'OVERBOOK', 'VEHICLE')),
    description TEXT,
